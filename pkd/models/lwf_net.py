...
class LwFNet(nn.Module):
    ...
    def get_angular_output(self, images, current_step=0):
        feature_maps = self.backbone(images)
        features = self.encoder_feature(feature_maps)
        if isinstance(current_step, list):
            c_s = current_step[-1]
            wf = F.linear(F.normalize(features, p=2, dim=1), F.normalize(self.classifier_dict[f'step:{c_s}'].classifier.weight, p=2, dim=1))
            return wf
        else:
            wf = F.linear(F.normalize(features, p=2, dim=1), F.normalize(self.classifier_dict[f'step:{current_step}'].classifier.weight, p=2, dim=1))
            return wf