...
from pkd.losses import CrossEntropyLabelSmooth, TripletLoss, ArcFaceLabelSmooth, AngularPenaltySMLoss
...
class BasePatchKD(object):
    ...
    def __init__(self, config, loader):
        ...
        if self.mode != 'visualize':
            ...
            #if self.config.gpu_id == 0: make_dirs(current_dir)
        # Train Configuration
        # resume_train_dir
    ...
    def _init_device(self):
        ...
        #self.device = torch.cuda.set_device(self.config.gpu_id)
    ...
    def _init_model(self):
        ...
        #self.model_dict['patchnet'] = DDP(self.model_dict['patchnet'])
        #self.model_dict['tasknet'] = DDP(self.model_dict['tasknet'])
    ...
    def _init_criterion(self):
        ...
        self.angular_criterion = AngularPenaltySMLoss(loss_type=self.config.loss_type, s=self.config.loss_s, m=self.config.loss_m)
        #self.arcface_criterion = ArcFaceLabelSmooth(num_classes=self.pid_num, embedding_size=128)
    ...
    def save_model(self, save_step, save_epoch):
        ...
        for module_name, model in self.model_dict.items():
    ...
    def resume_from_model(self, models_dir):
        ...
        '''resume model from resume_epoch'''
        for module_name, module in self.model_dict.items():
            model_path = os.path.join(models_dir, f'model_{module_name}_30.pkl')
