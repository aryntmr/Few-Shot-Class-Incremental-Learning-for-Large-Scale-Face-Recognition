...
class IncrementalSamples4vggface(IncrementalFaceSamples):
    def __init__(self, dataset_root, relabel=True):
        self.dataset_root = os.path.join(dataset_root, 'VGGFace')
    ...
    def get_image_paths_and_labels(self):
        ...
        for i, person_dir in enumerate(os.listdir(self.dataset_root)):
            ...
            if os.path.isdir(person_path):
                ...
                random.shuffle(person_all_image_path)
                selected_images = person_all_image_path[:max_images_per_person] if len(person_all_image_path) >= max_images_per_person else person_all_image_path
                for image_file in os.listdir(person_path):
                    if image_file.endswith('.jpg'):
                        if self.relabel:
                            ...
                            pid = int(person_dir)
                train_images, test_images = train_test_split(selected_images, test_size=0.3, random_state=42)
                for tr_img_path in train_images:
                    tr_data.append([tr_img_path, pid, 'vggface'])
                query_img_idx = random.randint(0, len(test_images)-1)
                for j, img_path in enumerate(test_images):
                    if j == query_img_idx: 
                        query_data.append([img_path, pid, 'vggface'])
                    else:
                        gallery_data.append([img_path, pid, 'vggface'])
