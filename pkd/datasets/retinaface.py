...
class IncrementalSamples4retinaface(IncrementalFaceSamples):
    def __init__(self, dataset_root, relabel=True):
        self.dataset_root = os.path.join(dataset_root, 'ms1m_small')
        ...
        for i, person_dir in enumerate(os.listdir(self.dataset_root)):
            ...
            if os.path.isdir(person_path):
                ...
                for tr_img_path in train_images:
                    tr_data.append([tr_img_path, pid, 'retinaface'])
                query_img_idx = random.randint(0, len(test_images)-1)
                for j, img_path in enumerate(test_images):
                    if j == query_img_idx: 
                        query_data.append([img_path, pid, 'retinaface'])
                    else:
                        gallery_data.append([img_path, pid, 'retinaface'])
