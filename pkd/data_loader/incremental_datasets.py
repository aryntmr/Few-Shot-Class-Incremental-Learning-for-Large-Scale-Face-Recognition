...
class IncrementalFaceSamples:
    ...
    def _show_info(self, train, query, gallery, name=None, if_show=True):
        if if_show:
            ...
            table = PrettyTable(['set', 'images', 'identities'])
            table.add_row([self.__class__.__name__ if name is None else name, '', ''])
            table.add_row(['train', str(train_info[0]), str(train_info[1])])
            table.add_row(['query', str(query_info[0]), str(query_info[1])])
            table.add_row(['gallery', str(gallery_info[0]), str(gallery_info[1])])
            print(table)
        else:
            pass
...
def Incremental_combine_train_samples(samples_list):
    ...
    all_pid_per_step, output_all_per_step = OrderedDict(), defaultdict(dict)
    max_pid = 0
    for step, samples in enumerate(samples_list):
        for a_sample in samples:
            ...
            try:
                dataset_name = a_sample[2]
                global_pid = max_pid + a_sample[1]
                # global_cid = str(dataset_name) + ':' + str(a_sample[2])
            except:
                print(a_sample)
                assert False
            all_samples.append([img_path, global_pid, dataset_name, local_pid])
    ...
    return all_samples, all_pid_per_step
